angular.module('pelenio').directive('uiCurrency', function ($filter, $parse) {
    return {
        require: 'ngModel',
        restrict: 'A',
...
/*
            ...
            ngModel.$render = function() {
                ...
                if (clean[clean.length-1] === ',') {
                     currencyValue = 'R$' + $filter('number')(parseFloat(clean)) + ',';

                } else if (clean.indexOf(',') != -1 && dotSplit[dotSplit.length - 1].length == 1) {
                    currencyValue = 'R$' + $filter('number')(parseFloat(clean), 1);
                } else if (clean.indexOf(',') != -1 && dotSplit[dotSplit.length - 1].length == 1) {
                    currencyValue = 'R$' + $filter('number')(parseFloat(clean), 2);
                } else {
                     currencyValue = 'R$' + $filter('number')(parseFloat(clean));
                }
